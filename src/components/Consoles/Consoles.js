...
class Consoles extends Component {
    ...
    async componentDidMount() {
        ...
        await API.get('/consoles/distinct')
            .then(res => {
    ...
    searchItem = async() =>
    {
        if(this.state.searchTerm.length == 0 || this.state.searchTerm.length> 2)
            await API.get(`/consoles/?UpdatedAt-asc;${this.state.query}`)
                .then(res => {
                    const consoles = res.data;
                    this.setState({consoles : consoles});
                })
                            ...
                            <table className="table is-fullwidth is-hoverable">
                                ...
                                {this.state.consoles.map(console =>
                                    <tr key={console.id}>
                                        ...
                                        <td>
                                            <p><strong>{console.title}</strong></p>
                                            <p>{console.description}</p>
                                            <span className="tag">{console.category}</span>
                                        </td>
                                        <td style={{width: "150px"}}>
                                            <p className="is-size-4"><strong>€{console.sellingPrice}</strong></p>
                                            <br/>
                                            <NavLink to={"/product/consoles/" + console.id} className="button is-light" style={{color: "#0096e3"}}><strong>Go to product</strong></NavLink>
                                        </td>
                                    </tr>)}
